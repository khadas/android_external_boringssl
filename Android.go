...
func init() {
	//该打印会在执行mm命令时，打印在屏幕上
	fmt.Println("libcrypto_defaults want to conditional Compile")
	android.RegisterModuleType("cc_libcrypto", DefaultsFactory)
}
...
func globalDefaults(ctx android.BaseContext) ([]string) {
    ...
    fmt.Println("libcrypto-NO_USE_RDRAND:",ctx.AConfig().Getenv("TARGET_BOARD_PLATFORM")) //通过 strings.EqualFold 比较字符串，可参考go语言字符串对比

    if (strings.EqualFold(ctx.AConfig().Getenv("TARGET_BOARD_PLATFORM"),"rk3126c")) {
	//添加 DEBUG 宏定义
        cppflags = append(cppflags,"-DNO_USE_RDRAND")
    }
    //将需要区分的环境变量在此区域添加 //....
    return cppflags
}
